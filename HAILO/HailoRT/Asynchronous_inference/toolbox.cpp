    ...
    PostprocessCallback postprocess_callback)
{
    ...
    int width  = static_cast<int>(org_width);
    int height = static_cast<int>(org_height);
    ...
    std::string pipeline =
        ...
        "udpsink host=192.168.1.49 port=5000";
    video.open(
        pipeline,
        cv::CAP_GSTREAMER,
        0,
    ...
    while (results_queue->pop(output_item)) {
        ...
        if (postprocess_callback &&
            !output_item.output_data_and_infos.empty()) {

            postprocess_callback(
                frame,
                output_item.output_data_and_infos
            );
        }
